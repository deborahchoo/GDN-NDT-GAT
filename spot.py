...
class SPOT:
    ...
    def initialize(self, level=0.98, min_extrema=False, verbose=True):
        ...
        S = np.sort(self.init_data)  # X is sorted to get the empirical quantile
        ...
        for i in tqdm.tqdm(range(self.data.size)):
            ...
            if not dynamic:
                ...
                if self.data[i] > self.extreme_quantile:
                    # if alarm, put it in the alarm list
                    if with_alarm:
                        alarm.append(i)
                    # otherwise, add it in the peaks
                    else:
    ...
    def run(self, with_alarm=True, plot=True):
        ...
        for i in tqdm.tqdm(range(self.data.size)):
            ...
            # case where the value exceeds the initial threshold but not the alarm ones
                ...
                # if alarm, we put it in the alarm list
                if with_alarm:
                    alarm.append(i)
                # otherwise, add it in the peaks
                else:
